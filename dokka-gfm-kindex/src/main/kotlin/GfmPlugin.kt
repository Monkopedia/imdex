...
open class CommonmarkRenderer(
    ...
    override fun StringBuilder.buildHeader(
        level: Int,
        node: ContentHeader,
    ...
    private fun StringBuilder.buildPlatformDependentItem(
        ...
        if (content is ContentGroup &&
            content.children.firstOrNull { it is ContentTable } != null
        ) {
    ...
    override fun StringBuilder.buildResource(
    ...
    override fun StringBuilder.buildTable(
        ...
        if (node.dci.kind == ContentKind.Sample || node.dci.kind == ContentKind.Parameters) {
            ...
            node.children.forEach {
                ...
                append(" | ".repeat((size - it.children.size).coerceAtLeast(0)))
                append("\n")
            }
        }
    }
    ...
    override fun buildPage(page: ContentPage, content: (StringBuilder, ContentPage) -> Unit):
        String = buildString {
            content(this, page)
        }
    ...
    override fun buildError(node: ContentNode) {
        context.logger.warn(
            "Markdown renderer has encountered problem. The unmatched node is $node"
        )
    }
    ...
    override suspend fun renderPage(page: PageNode) {
        ...
        when (page) {
            ...
            is RendererSpecificPage -> when (val strategy = page.strategy) {
                ...
                "Page ${page.name} cannot be rendered by renderer as it is not renderer " +
...
class MarkdownLocationProviderFactory(val context: DokkaContext) : LocationProviderFactory {
    ...
    override fun getLocationProvider(pageNode: RootPageNode) = MarkdownLocationProvider(
        pageNode,
        context
    )
}
