...
class SearchOverlay : LifecycleComponent<SearchOverlayProps, SearchOverlayState>() {
    private val searchQueue = CoroutineQueue()
    private val title = Mutable("" as CharSequence)
    ...
    override fun RBuilder.render() {
        holder.lifecycle = props.title?.onValue {
            println("Search bar title: $it")
            title.value = it.toString()
        }
        child(AppBarComponent::class) {
            attrs {
                title = this@SearchOverlay.title
                onSearchChanged = {
                    if (it != state.query) {
                        setState {
                            query = it
